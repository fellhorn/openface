...
def findClosestFeatures(target, features, num):
    distances = []

    for key in features:
        d = target - features[key]["rep"]
        l2compare = np.dot(d, d)
        distances.append(l2compare)
...
boost = True
# decide that a face is the same if it belongs to the n closest faces
decisionLevel = 10
        ...
        for row in reader:
            if (row[0] in representations) and (row[1] in representations):
                ...
                if boost and l2 < 0.9:
                    ...
                    distances = findClosestFeatures(first, representations,
                                                    decisionLevel)
                    if l2 in distances:
                        # it is among th eclosest distances
                        classification = 1
                        proba = 1.0
                    else:
                        # it is not
