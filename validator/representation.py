...
for (img) in (args.imgs):
    try:
        rep = getRep(img)
        personName = ''.join(img.split('/')[-1].split('_')[:-1])
        print("{};{};{}".format(img, personName, np.sum(rep)))

        representations.append({'name': personName, 'rep': rep})
    except Exception:
...
for (rep1, rep2) in itertools.combinations(representations, 2):
    d = rep1['rep'] - rep2['rep']
    features.append(d)
    l2 = np.dot(d, d)
    classification = clf.predict([d])
    ...
    if rep1['name'] == rep2['name']:
        ...
        labels.append(0)
        # if (lastSameD != []):
        #     features.append(lastSameD[0])
        #     labels.append(1)
        if verbose:
            print("Should be different")

#createHistogram(same, "same")
#createHistogram(different, "different")
#findDecisionBorder(same, different)

with open('features.json', 'wb') as featurefile:
    featurefile.write(pd.Series(features).to_json(orient='values'))

with open('labels.json', 'wb') as labelfile:
    labelfile.write(pd.Series(labels).to_json(orient='values'))

with open('weights.json', 'wb') as labelfile:
    labelfile.write(pd.Series(weights).to_json(orient='values'))
